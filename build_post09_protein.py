"""Карусель №9 «Сколько белка тебе реально нужно» (6 слайдов, 1080×1350).

Фирменный стиль POWERELIX через brand_overlay (как посты №1-6).
Акцент — фирменная мята #16FFB3. Тема: норма белка и как её добрать.
...
DDARK = (10, 20, 16)
SRC = "/private/tmp/claude-501/-Users-maksimrogoznikov-projects/f76aea07-d110-4ec0-ae31-6649558b262e/scratchpad/p09"
OUT = "output/Карусели/post09_белок"
...
def final_slide(path, scene, heading, lines_txt, disclaimer):
    # верх затемняем сильно: белый текст на светлой фуд-съёмке иначе не читается
    img = _darken(_cover(Image.open(scene)), base=58, top=760, bottom=220)
    d = ImageDraw.Draw(img)
    _mark(d, light=True)
    y = 230
...
cover(f"{OUT}/01.png", f"{SRC}/cover.png",
      ...
      "Спойлер: скорее всего, ты недобираешь — и не догадываешься")

def num_slide(path, kicker, heading, rows, note=None):
    """Слайд с числами: крупная цифра слева, расшифровка справа."""
    img = _canvas()
    d = ImageDraw.Draw(img)
    _mark(d)
    y = 230
    _spaced(d, (M, y), kicker, _font(MONT_BLACK, 34), ACCENT, 4)
    y += 58
    ...
    for big, small in rows:
        fb = _font(MONT_BLACK, 52)
        d.text((M, y), big, font=fb, fill=ACCENT)
        bw = d.textlength(big, font=fb)
        fs = _font(INTER_MED, 34)
        ty = y + 12
        for ln in _wrap(d, small, fs, W - 2 * M - bw - 40):
            d.text((M + bw + 34, ty), ln, font=fs, fill=INK); ty += 44
        y = max(y + 76, ty + 14)
    if note:
        y += 12
        for ln in _wrap(d, note, _font(INTER_MED, 32), W - 2 * M):
...
num_slide(f"{OUT}/02.png", "НОРМА", "Считается от веса, а не «на глаз»",
          [("0,8 г/кг", "минимум, чтобы организм не начал разбирать мышцы"),
           ("1,2–1,6 г/кг", "если тренируешься, худеешь или тебе за 40")],
          note="При весе 70 кг это 84–112 г белка в день. Не «кусок мяса», а конкретная цифра.")

num_slide(f"{OUT}/03.png", "В ЕДЕ", "Сколько это на тарелке",
          [("46 г", "куриная грудка, 150 г"),
           ("34 г", "творог 5%, 200 г"),
           ("18 г", "три яйца"),
           ("8 г", "гречка, 200 г готовой")],
          note="Чтобы набрать норму, нужно 3–4 полноценных белковых приёма, а не один ужин.")

rules_slide(f"{OUT}/04.png", "Обычный день без белка:",
            ["Кофе с булкой — 3 г",
             "Салат с курицей на обед — 20 г",
             "Паста вечером — 12 г"],
            note="Итого 35 г из нужных ста. Кажется, что «ел нормально» — а по факту треть нормы.")

rules_slide(f"{OUT}/05.png", "Как тело сообщает о нехватке:",
            ["Волосы лезут, ногти слоятся",
             "Тянет на сладкое во второй половине дня",
             "Вес стоит, а мышцы уходят",
             "Царапины заживают дольше обычного",
             "Через час после еды снова голодно"],
            note="Белок даёт сытость и «строительный материал» — без него тело экономит на "
                 "волосах, коже и мышцах.")

final_slide(f"{OUT}/06.png", f"{SRC}/final.png",
            "Белок — из тарелки",
            ["Добавка не заменит нормальную еду.",
             "Но коллаген — тот белок, которого в рационе почти нет:",
             "мы не едим хрящи и кожу, а держат они многое.",
