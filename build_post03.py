"""Сборка карусели Пост №3 «Вечно на нервах?» (Магний + B6, 7 слайдов, 1080x1350).

Структура как у Постов №1/№2, тема — нервы/стресс/сон: фиолетовый акцент (#8975BF).
Hero-кадры (обложка + продукт) — gpt-image-2 (`post03_s1.png`, `post03_s6.png`).
...
ACCENT = _hex("#8975BF")  # магний — фиолетовый (нервы под контролем)
DDARK = (14, 10, 22)      # тёмно-фиолетовый для затемнения фото-слайдов
OUT = "output/post03"
...
# ── слайды ──
cover(f"{OUT}/01.png", "output/scenes/post03_s1.png",
      "Вечно на нервах?",
      "Почему ты на взводе — и что реально успокаивает", "СОХРАНИ  →")
text_slide(f"{OUT}/02.png", "Знакомо?", bullets=[
    "Раздражает любая мелочь",
    "Не можешь уснуть — мысли по кругу",
    "Просыпаешься среди ночи",
    "Сводит ноги по ночам",
])
text_slide(f"{OUT}/03.png", "Дело не в характере.",
           big="Тебе не хватает магния.",
           note="Стресс вымывает его первым.")
text_slide(f"{OUT}/04.png", "Что вымывает магний:", bullets=[
    "Хронический стресс",
    "Кофе и сахар",
    "Алкоголь",
    "Спорт и пот",
...
text_slide(f"{OUT}/05.png", "Что успокаивает нервы:", bullets=[
    "Сон по режиму",
    "Меньше кофе вечером",
    "Прогулки на воздухе",
    "Магний с B6",
])
hero_product_slide(f"{OUT}/06.png", "output/scenes/post03_s6.png",
                   "Магний + B6 — нервы под контролем",
                   "Спокойный сон, защита от стресса и здоровое сердце.",
                   disclaimer="БАД. Не является лекарственным средством. Есть противопоказания.")
text_slide(f"{OUT}/07.png", "Хочешь спать спокойно?",
           note="Магний + B6 от POWERELIX поддержит нервную систему — спокойный сон "
                "и меньше стресса. Ищи по ссылке в профиле.\n"
                "А что чаще выбивает из равновесия тебя? Пиши в комментариях.",
           cta="Сохрани · Подписывайся — про здоровье по-простому")
