...
def _person_phrase(model_key: str) -> str:
    from .brand import CHILD_KEYS
    if model_key in CHILD_KEYS:
        return f"ребёнок ({CHILD_KEYS[model_key]}), радостный, здоровый, БЕЗ конкретного лица-референса"
...
def generate_post_assets(post_id: int, ratio: Optional[str] = None, extra: str = "") -> Optional[int]:
    ...
    if existing_user_refs and not extra:
        ...
        prompt = (
            ...
            f"НО: если в кадре есть человек — замени его на {_person_img} "
            "(поза и действие как в референсе, НЕ копируй внешность человека из референса). "
            "Одежда — ТА ЖЕ, что на референсе (тип, цвет, фасон, посадка)."
            + (f" ЛЮБОЙ продукт/упаковку замени на НАШ продукт с {_prod_ord} изображения — форма банки, "
        ...
        prompt = (
            ...
            f"композиция, поза, ракурс, действие, свет, стиль. Человек — {_person_same}."
            + (f" Продукт в руках — банка СТРОГО как на {_prod_ord2} изображении (это настоящий продукт "
               ...
               f"продукта, повёрнута к камере, банка целиком в кадре. ЗАПРЕЩЕНО придумывать другой "
               f"бренд, другой текст на этикетке или другую упаковку — если сомневаешься, ориентируйся "
               f"на {_prod_ord2} изображение, а не на первое."
               if prod_ref else "")
...
ДЛИНА подписи — 800-950 знаков: используй объём по максимуму (богатый, полезный текст), но НЕ
превышай 950 знаков, иначе не влезет в подпись Telegram под картинкой (жёсткий лимит 1024 с учётом артикула).
...
# добавляются ПОСЛЕ — резерв под них. 900 = 1024 − артикул(~40) − ссылки канала(~46) − запас.
CAPTION_DESC_LIMIT = 900
...
НЕ упоминай СПОСОБ ПРИМЕНЕНИЯ и дозировку (сколько капсул, когда/как принимать, длительность
курса, сколько в упаковке) — этого в сценарии быть НЕ должно (фокус на пользе, эмоции, результате).
Верни строго структуру по схеме."""
