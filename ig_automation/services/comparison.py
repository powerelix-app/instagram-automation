...
def _execute_aicopy(comparison_id: int, product_ids: List[str], ratio: str = "4:5") -> None:
    ...
    prompt = (
        ...
        "Если наша банка стоит в РЯДУ с другими иконками одинакового размера (круги/плашки ингредиентов) — "
        "впиши банку в ТАКОЙ ЖЕ размер и выравнивание, что и соседние иконки (одна базовая линия, одинаковая "
        "высота ячейки), чтобы ряд смотрелся единой аккуратной сеткой, а банка не торчала выше/ниже соседей. "
        ...
        "премиально (не кислотно). НЕ рисуй никаких логотипов и вордмарков — фирменный тег добавим сами "
        "отдельно; ОСТАВЬ левый верхний угол свободным под него. "
        "УБЕРИ любой чужой бренд, лого, водяной знак, значки соцсетей и посторонние badge. "
    ...
    final.save(out_path)
    try:
        producer.stamp_wordmark(out_path)   # единый фирменный тег-пилюля (не запечённый AI вордмарк)
    except Exception as e:
        log.warning("stamp_wordmark failed for #%s: %s", comparison_id, e)
    with session_scope() as s:
        c = s.get(Comparison, comparison_id)
        if c:
            c.output_path = f"/media/comparisons/{out_path.name}"
...
def _gen_caption(comparison_id: int) -> None:
    ...
    try:
        ...
        prompt = (f"Напиши короткое (2-3 предложения) описание-подпись к инфографике бренда БАД POWERELIX "
                  ...
                  f"по-русски. Добавь по смыслу немного эмодзи-иконок (в начале ключевых мыслей или строк — "
                  f"например 🌿💧⚡️✨🧬🔥), чтобы текст читался живее, но без перебора (1 иконка на мысль, "
                  f"не подряд). Без хэштегов и без артикулов — их добавлю отдельно.")
        client = anthropic.Anthropic()
