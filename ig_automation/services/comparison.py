...
def _execute_aicopy(comparison_id: int, product_ids: List[str], ratio: str = "4:5") -> None:
    ...
    n = len(product_ids)
    prompt = (
        "ПЕРВОЕ изображение — референс-ВДОХНОВЕНИЕ (НЕ копировать один-в-один!). Возьми его КОНЦЕПЦИЮ, "
        "идею и общую СТРУКТУРУ (какие блоки, их смысл и порядок: напр. 2 колонки сравнения / симптом→"
        "продукт / список с иконками / сетка со связями) как ОСНОВУ, но сделай СОБСТВЕННЫЙ, узнаваемо-"
        "брендовый дизайн POWERELIX — это НАША самостоятельная работа ПО МОТИВАМ, НЕ клон. Сохрани СУТЬ и "
        "посыл, но ЯВНО ИЗМЕНИ визуальное исполнение: свой фон и текстуру (та же тональность/настроение, "
        "но иначе), свой стиль иконок, рамок, кружков и декора, свою типографику заголовков, свою компоновку "
        "акцентов и декоративных элементов — чтобы дизайн заметно отличался от референса и читался как "
        "оригинал POWERELIX. "
        f"Замени товары на НАШИ РОВНО {n} банок бренда POWERELIX из следующих изображений — форма, крышка, "
        "цвет и ЭТИКЕТКА строго как на этих референсах, каждая банка целиком в кадре. "
        f"Товаров должно быть ровно {n}: если у референса слотов под товар больше — АДАПТИРУЙ раскладку под "
