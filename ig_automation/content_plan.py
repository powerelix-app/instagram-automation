...
# План — это СКЕЛЕТ (что/когда/о чём), а не готовый контент. Полный текст подписи и
# хэштеги генерятся ПОЗЖЕ по каждому посту отдельно (кнопка «текст» на странице поста).
# Иначе один вызов на 21 пост с полными подписями зависает на Haiku/ProxyAPI.
class Post(BaseModel):
    ...
    idea: str = Field(description="Короткая суть поста: о чём он, ключевая мысль/структура (1-2 предложения, НЕ полная подпись)")
...
Твоя задача — составить СКЕЛЕТ контент-плана (что/когда/о чём каждый пост), а НЕ готовые тексты. Полные подписи пишутся потом отдельно по каждому посту. По каждому посту дай только: тип, формат, тему-рубрику, продукт, цепляющий хук и короткую суть (idea). Упор на пользу и вовлечение, не на прямые продажи в каждом посте.
...
- Хук — цепляющий, живой, на «ты». idea — 1-2 предложения сути (не полный текст).
