"""Reels №4 «Вечерний ритуал на сон» (Магний + B6) — уютный вечерний рекламный ролик.

Бессонница/нервы → магний → спокойный сон. 6 кадров (тёплая вечерняя эстетика)
оживляются Grok (Replicate), мягкая озвучка (OpenAI TTS), wordmark POWERELIX.
1080×1920, ~18 сек. Музыку добавить в IG.
"""
import os
import subprocess
...
WHITE = (255, 255, 255)
ACCENT = (160, 140, 210)  # светло-фиолетовый (магний) для тёмного фона
GREY = (215, 210, 225)

SHOTS = [
    ("m_1tense", "subtle restless motion, she shifts slightly and breathes, very slow gentle "
                 "push-in, warm dim light", 3, "Мысли по кругу?", None, False),
    ("m_2take", "she calmly takes a capsule and sips water, gentle slow serene motion", 3, None,
                None, False),
    ("m_3jar", "warm lamp glow flickers softly, very slow gentle push-in on the jar", 2, None,
               None, False),
    ("m_4relax", "she relaxes with eyes gently closing, soft slow breathing, letting go", 3,
                 "Тело отпускает день", None, False),
    ("m_5sleep", "she sleeps peacefully, very subtle breathing, slow gentle, restful", 3,
                 "Спокойный сон", None, False),
    ("m_6pack", "warm lamp glow, very slow gentle push-in on the jar", 4, None,
                "Спокойный сон · каждый вечер", True),
]

VO_TEXT = ("Мысли крутятся, и никак не уснуть? Знакомо. Дай нервам наконец выдохнуть. "
           "Магний с витамином B6 от POWERELIX мягко поддержит нервную систему — "
           "спокойный сон, меньше стресса, и утро добрее. Засыпай легко, каждый вечер.")
VO_VOICE = "shimmer"
...
def overlay_png(path, main, slogan, disclaimer):
    ...
    if main or slogan or disclaimer:
        ...
        a = np.clip(np.where(ys > H - 700, (ys - (H - 700)) / 700 * 180, 0), 0, 180).astype("uint8")
        shade = Image.fromarray(np.repeat(a, W, axis=1).reshape(H, W))
        ov.paste(Image.new("RGBA", (W, H), (10, 8, 18, 255)), (0, 0), shade)
    d = ImageDraw.Draw(ov)
